...
describe('quantity and resupply rules', () => {
  ...
  it('adding a threshold or re-enabling prompts handles already-low stock', () => {
    ...
    const edited = applyCommand(
      data,
      { type: 'edit', id, input: { ...itemInput(data.items[0]), resupply_threshold: 2 } },
      later,
    );
    expect(edited.items[0].resupply_flag).toBe(true);
  ...
  it('records a partial purchase atomically without immediately reflagging', () => {
    ...
    expect(next.quantity_events.at(-1)).toMatchObject({
      reason: 'purchase',
      before: { quantity: 0 },
      after: { quantity: 0.5 },
    });
    expect(
      applyCommand(data, { type: 'purchase', id, amount: 0.5, keep: true }, later).items[0].resupply_flag,
    ).toBe(true);
  });
...
describe('units and qualitative display', () => {
  ...
  it('converts quantity and threshold together while preserving historical units', () => {
    ...
    expect(next.quantity_events[0].after).toMatchObject({
      quantity: 0.5,
      unit: 'bag',
      package_size: { amount: 1, unit: 'kg' },
    });
...
describe('duplicates, history, and undo', () => {
  ...
  it('merges into an explicitly counted total and can undo the merge', () => {
    ...
    const two = applyCommand(
      ...
      {
        type: 'create',
        input: { ...newItemInput(), name: 'Tomatoes, canned', quantity: 2, resupply_threshold: 3 },
      },
      later,
    );
  ...
  it('undo restores the previous quantity and flag with a compensating event', () => {
    ...
    expect(reverted.quantity_events.at(-1)).toMatchObject({
      reason: 'undo',
      undo_of_event_id: bought.quantity_events.at(-1)?.id,
    });
