...
describe('quantity and resupply rules', () => {
  it('raises at equality and keeps the flag when a recount increases stock', () => {
    ...
    expect(low.items[0].flagged).toBe(true);
    const recounted = applyCommand(low, { type: 'actual', id, quantity: 7 }, later);
    expect(recounted.items[0].flagged).toBe(true);
    expect(recounted.items[0].verified_at).toBe(later);
    expect(data.items[0].quantity).toBe(4);
  });
  it('supports zero thresholds and initial-low items', () => {
    expect(setup({ quantity: 0, threshold: 0 }).data.items[0].flagged).toBe(true);
    expect(setup({ quantity: 0, threshold: null }).data.items[0].flagged).toBe(false);
  });
  it('never-prompt blocks automatic flags while allowing deliberate manual flags', () => {
    ...
    expect(low.items[0].flagged).toBe(false);
    expect(applyCommand(low, { type: 'flag', id, value: true }, later).items[0].flagged).toBe(true);
  });
  it('enabling never-prompt preserves an existing need', () => {
    const { data, id } = setup({ quantity: 1 });
    const edited = applyCommand(
      data,
      { type: 'edit', id, input: { ...itemInput(data.items[0]), never_prompt: true } },
      later,
    );
    expect(edited.items[0].flagged).toBe(true);
  });
  it('does not re-raise a canceled flag while still below threshold', () => {
    ...
    expect(lower.items[0].flagged).toBe(false);
    expect(parseBackup(JSON.stringify(lower)).items[0].flagged).toBe(false);
  });
  it('adding a threshold or re-enabling prompts handles already-low stock', () => {
    const { data, id } = setup({ quantity: 1, threshold: null });
    const edited = applyCommand(
      data,
      { type: 'edit', id, input: { ...itemInput(data.items[0]), threshold: 2 } },
      later,
    );
    expect(edited.items[0].flagged).toBe(true);
    ...
    expect(
      applyCommand(
        never.data,
        { type: 'edit', id: never.id, input: { ...itemInput(never.data.items[0]), never_prompt: false } },
        later,
      ).items[0].flagged,
    ).toBe(true);
  });
  it('records a partial purchase atomically without immediately reflagging', () => {
    ...
    expect(next.items[0]).toMatchObject({ quantity: 0.5, flagged: false, verified_at: null });
    ...
    expect(
      applyCommand(data, { type: 'purchase', id, amount: 0.5, keep: true }, later).items[0].flagged,
    ).toBe(true);
  });
  it('only physical checks update verified_at, including an unchanged check', () => {
    const { data, id } = setup();
    const renamed = applyCommand(
      data,
      { type: 'edit', id, input: { ...itemInput(data.items[0]), name: 'Salted butter' } },
      later,
    );
    expect(renamed.items[0].verified_at).toBeNull();
    const checked = applyCommand(renamed, { type: 'actual', id, quantity: 4 }, later);
    expect(checked.items[0].verified_at).toBe(later);
    expect(checked.events).toHaveLength(1);
  });
  it('rejects invalid changes without changing the input state', () => {
    const { data, id } = setup();
    for (const delta of [-5, NaN, Infinity])
      expect(() => applyCommand(data, { type: 'adjust', id, delta }, later)).toThrow();
...
describe('units and qualitative display', () => {
  ...
  it('converts quantity and threshold together while preserving historical units', () => {
    ...
    const next = applyCommand(
      data,
      { type: 'convert', id, factor: 1000, unit: 'g', package_size: null },
      later,
    );
    ...
    expect(next.events[0].after).toMatchObject({
      quantity: 0.5,
      unit: 'bag',
      package_size: { amount: 1, unit: 'kg' },
    });
    expect(next.events.at(-1)?.reason).toBe('unit_change');
  });
  it('blocks silently redefining units or occupied packages', () => {
    const { data, id } = setup({ unit: 'bag', package_size: { amount: 1, unit: 'kg' } });
    expect(() =>
      applyCommand(data, { type: 'edit', id, input: { ...itemInput(data.items[0]), unit: 'kg' } }, later),
    ).toThrow('Change unit');
    expect(() =>
      applyCommand(
        data,
        { type: 'edit', id, input: { ...itemInput(data.items[0]), package_size: { amount: 2, unit: 'kg' } } },
        later,
      ),
    ).toThrow('package');
  });
  it('rejects contradictory physical conversion factors and dimensions', () => {
    const { data, id } = setup({ unit: 'bag', package_size: { amount: 1, unit: 'kg' } });
    expect(() =>
      applyCommand(data, { type: 'convert', id, factor: 1, unit: 'g', package_size: null }),
    ).toThrow('conversion factor');
    expect(() =>
      applyCommand(data, { type: 'convert', id, factor: 1, unit: 'l', package_size: null }),
    ).toThrow('Mass and volume');
    expect(() =>
      applyCommand(data, {
        type: 'convert',
        id,
...
describe('duplicates, history, and undo', () => {
  ...
  it('merges into an explicitly counted total and can undo the merge', () => {
    ...
    const two = applyCommand(
      ...
      {
        type: 'create',
        input: { ...newItemInput(), name: 'Tomatoes, canned', quantity: 2, threshold: 3 },
      },
      later,
    );
    ...
    expect(merged.items[0]).toMatchObject({
      quantity: 2,
      flagged: true,
      aliases: ['Tomatoes, canned'],
    });
    expect(merged.items[1]).toMatchObject({ deleted_at: later, merged_into: id });
    expect(merged.events.some((event) => event.item_id === source)).toBe(true);
    ...
    expect(undone.items[0].flagged).toBe(false);
  ...
  it('undo restores the previous quantity and flag with a compensating event', () => {
    ...
    expect(reverted.items[0]).toMatchObject({ quantity: 0, flagged: true });
    expect(reverted.events.at(-1)).toMatchObject({
      reason: 'undo',
      undo_of_event_id: bought.events.at(-1)?.id,
    });
    expect(reverted.events).toHaveLength(3);
  });
  it('undoing creation uses a tombstone rather than destroying the record', () => {
    const before = emptyInventory(now);
    const created = applyCommand(
      before,
      { type: 'create', input: { ...newItemInput(), name: 'Coffee' } },
      now,
    );
  ...
  it('prunes expired events but retains quantities and deleted item identities', () => {
    ...
    expect(pruned.events).toHaveLength(0);
...
describe('backup validation', () => {
  ...
  it('rejects imported quantities that would display a tiny positive amount as zero', () => {
    const { data } = setup();
    expect(() => validateInventory({ ...data, items: [{ ...data.items[0], quantity: 0.00000001 }] })).toThrow(
      'six decimal',
    );
  });
});
