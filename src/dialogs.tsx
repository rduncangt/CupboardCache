...
function readPackage(form: FormData): PackageSize | null {
  return optional(form, 'package_amount')
    ? {
        amount: numeric(form, 'package_amount'),
        unit: String(form.get('package_unit')) as PackageSize['unit'],
      }
    : null;
}
...
export function ItemEditor({
  ...
  commit,
  onExisting,
}: Common & { data: InventoryData; item?: Item; onExisting: (item: Item) => void }) {
  const initial = item ? itemInput(item) : newItemInput();
  const [name, setName] = useState(initial.name);
  const [unit, setUnit] = useState(initial.unit);
  const [displayMode, setDisplayMode] = useState(initial.display_mode);
  const [packageUnit, setPackageUnit] = useState<PackageSize['unit']>(initial.package_size?.unit ?? 'kg');
  const [dirty, setDirty] = useState(false);
  const [localError, setLocalError] = useState<string | null>(null);
  const candidates = duplicates(data.items, name, item?.id);
  const categories = [
    ...new Set(data.items.flatMap((item) => (item.category ? [item.category] : []))),
  ].sort();
  const locations = [
    ...new Set([
      'Pantry',
  ...
  return (
    <Modal title={item ? 'Edit item' : 'Add to your cupboard'} onClose={onClose} busy={busy} dirty={dirty}>
      <form
        ...
        onSubmit={async (event) => {
          ...
          try {
            ...
            if (
              await commit(
                item ? { type: 'edit', id: item.id, input } : { type: 'create', input },
                item ? 'Item updated' : `${input.name} added`,
              )
            )
              onClose();
          } catch (error) {
            setLocalError(error instanceof Error ? error.message : 'Check the item details.');
          }
        }}
      >
        ...
        {candidates.length > 0 && (
          <div class="duplicate-note">
            ...
            {candidates.map((candidate) => (
              <button key={candidate.id} type="button" onClick={() => onExisting(candidate)}>
                <span>{candidate.name}</span>
                <small>
                  {candidate.deleted_at
                    ? 'Archived · restore'
                    : `${number(candidate.quantity)} ${candidate.unit}`}
                </small>
                <Icon name="arrow" size={16} />
              </button>
            ))}
            <label class="check-field">
              <input type="checkbox" required /> This is a different item
            </label>
          </div>
        )}
        <div class="form-grid">
          ...
          <Field
            label="Unit"
            hint={item ? 'Use Change unit from item details to convert.' : 'Choose the way you count it.'}
          >
            <input
              name="unit"
              list="units"
        ...
        <div class="form-grid">
          <Field label={`Shopping threshold (${unit})`} hint="Leave blank to flag items yourself.">
            <input
              name="threshold"
              type="number"
              inputMode="decimal"
        ...
        <label class="check-field">
          <input name="never_prompt" type="checkbox" defaultChecked={initial.never_prompt} /> Never add
          automatically to shopping
        </label>
        <details
          class="form-details"
        ...
        >
          ...
          <Field label="Also known as" hint="Separate other names with commas.">
            <input
              name="aliases"
          ...
          <div class="form-grid">
            <Field
              label={`One ${unit} contains`}
              hint={
                item && item.quantity > 0
                  ? 'Keep this size while stock remains; use Change unit to convert.'
                  : 'Optional, for precise measurements.'
              }
            >
              <input
                name="package_amount"
                ...
                step="any"
                min="0.000001"
                defaultValue={initial.package_size?.amount ?? ''}
                placeholder="e.g. 1"
              />
            </Field>
            <Field label="Package measure">
              <select
                name="package_unit"
                value={packageUnit}
                onChange={(event) => setPackageUnit(event.currentTarget.value as PackageSize['unit'])}
              >
                {['g', 'kg', 'ml', 'l'].map((unit) => (
...
export function QuantityDialog({
  ...
  try {
    if (amount.trim()) converted = convertAmount(Number(amount), unit, item);
  } catch {
    ...
    >
      <div class="dialog-item">
        ...
        <div>
          <h3>{item.name}</h3>
          <p>
            Currently {number(item.quantity)} {item.unit}
          </p>
        </div>
      </div>
      <form
        onSubmit={async (event) => {
          ...
          try {
            ...
            if (
              await commit(
                mode === 'purchase'
                  ? { type: 'purchase', id: item.id, amount: quantity, keep }
                  : { type: 'actual', id: item.id, quantity },
                mode === 'purchase' ? `${item.name} restocked` : `${item.name} checked`,
              )
            )
              onClose();
          } catch (error) {
            setLocalError(error instanceof Error ? error.message : 'Check the quantity.');
          }
        }}
      >
        <div class="form-grid">
          <Field label={mode === 'purchase' ? 'Amount bought' : 'Actual quantity'}>
            <input
              type="number"
              inputMode="decimal"
              value={amount}
        ...
        {resulting !== null && (
          <div class="quantity-preview">
            ...
            <strong>
              {number(resulting)} <small>{item.unit}</small>
            </strong>
          </div>
        )}
        {mode === 'purchase' ? (
          <>
            <label class="check-field">
              ...
              />{' '}
              Keep on my shopping list
            </label>
            {low && (
              <p class="helper">
                This leaves you at or below your threshold. Keep it on the list if you still need more.
              </p>
            )}
          </>
        ) : (
          <p class="helper">
            A shelf correction keeps any existing shopping flag. Choose Record a purchase when you buy more.
          </p>
        )}
        <ErrorMessage message={localError || error} />
        <div class="modal-actions">
          <button class="button primary" disabled={busy}>
            {busy ? 'Saving…' : mode === 'purchase' ? 'Record purchase' : 'Save actual quantity'}
            <Icon name="check" size={18} />
          </button>
        </div>
      </form>
...
export function ItemDetails({
  ...
  item: Item;
  onEdit: () => void;
  onQuantity: (mode: 'actual' | 'purchase') => void;
  onConvert: () => void;
  onMerge: () => void;
}) {
  ...
  return (
    <Modal title={item.name} onClose={onClose} busy={busy}>
      ...
      <div class="button-row">
        ...
        <button class="button secondary" disabled={busy} onClick={() => onQuantity('purchase')}>
      ...
      {item.display_mode === 'qualitative' && (
        <div class="qualitative-controls">
            ...
            {(
              ...
              <button
                ...
                onClick={async () => {
                  if (
                    await commit(
                      { type: 'actual', id: item.id, quantity: setOpenContainer(item.quantity, fill) },
                      `${item.name} checked`,
                    )
                  )
                    onClose();
          ...
          <small>Spare containers stay counted. Use All out below for zero total.</small>
        </div>
      )}
      ...
      <div class="detail-tools">
        ...
        <button disabled={busy || data.items.filter((item) => !item.deleted_at).length < 2} onClick={onMerge}>
          <Icon name="merge" /> Merge a duplicate
        </button>
        <button
          disabled={busy || item.quantity === 0}
          onClick={async () => {
            if (await commit({ type: 'actual', id: item.id, quantity: 0 }, `${item.name} marked out`))
              onClose();
          }}
        >
          <Icon name="minus" /> All out
        </button>
        <button
          class="danger-text"
          disabled={busy}
          onClick={async () => {
            if (await commit({ type: 'delete', id: item.id }, `${item.name} archived`)) onClose();
          }}
        >
          <Icon name="trash" /> Archive item
        </button>
      </div>
      ...
      <section class="history">
        ...
        {events.length === 0 ? (
          ...
          (showAll ? events : events.slice(0, 10)).map((event) => (
            <div class="history-row" key={event.id}>
              <Icon
                name={event.reason === 'purchase' ? 'bag' : event.reason === 'undo' ? 'undo' : 'clock'}
                size={17}
              />
              <div>
                <strong>{event.reason.replace('_', ' ')}</strong>
                <span>
                  {event.before && `${number(event.before.quantity)} ${event.before.unit} → `}
                  {number(event.after.quantity)} {event.after.unit}
                  {event.item_id !== item.id && ' · merged item'}
                  {event.undo_of_event_id &&
        ...
        {events.length > 10 && !showAll && (
          <button class="text-button" onClick={() => setShowAll(true)}>
...
export function ConvertDialog({ item, busy, error, onClose, commit }: Common & { item: Item }) {
  ...
  const [dirty, setDirty] = useState(false);
  useEffect(() => {
    const suggested = suggestedPackage(item, Number(factor), unit);
    setPackAmount(String(suggested?.amount ?? ''));
    setPackUnit(suggested?.unit ?? 'g');
  }, [factor, unit]);
  return (
    <Modal title="Change measurement unit" onClose={onClose} busy={busy} dirty={dirty}>
      <p class="helper">
        Keep the amount on the shelf the same. For 1 kg bags changed to grams, one old bag becomes 1000 new
        grams. A spelling correction uses a factor of 1.
      </p>
      <form
        onInput={() => setDirty(true)}
        onSubmit={async (event) => {
              ...
              await commit(
                {
                  ...
                  unit: unit.trim(),
                  package_size: readPackage(new FormData(event.currentTarget)),
                },
                'Quantity and threshold converted',
              )
            )
              onClose();
          } catch (error) {
            setLocalError(error instanceof Error ? error.message : 'Check the conversion.');
          }
        }}
      >
        <div class="form-grid">
          ...
          <Field label={`New units in one old ${item.unit}`}>
            <input
              type="number"
              inputMode="decimal"
              value={factor}
        ...
        {Number(factor) > 0 && (
          <div class="conversion-preview">
            <p>
              ...
              <strong>
                {number(item.quantity * Number(factor))} {unit}
              </strong>
            </p>
            <p>
              Threshold:{' '}
        ...
        <div class="form-grid">
          <Field label="Amount in one new unit" hint="Leave blank if the new unit is already precise.">
            <input
              name="package_amount"
              type="number"
              inputMode="decimal"
              ...
              step="any"
              value={packAmount}
              onInput={(event) => setPackAmount(event.currentTarget.value)}
            />
          </Field>
          <Field label="Package measure">
            <select
              name="package_unit"
              value={packUnit}
              onChange={(event) => setPackUnit(event.currentTarget.value as PackageSize['unit'])}
            >
              {['g', 'kg', 'ml', 'l'].map((unit) => (
                <option value={unit}>{unit}</option>
              ))}
        ...
        <label class="check-field">
          <input type="checkbox" required /> The conversion and package size above describe the same stock.
        </label>
        <ErrorMessage message={localError || error} />
        <div class="modal-actions">
          <button class="button primary" disabled={busy}>
            {busy ? 'Saving…' : 'Convert unit'}
          </button>
        </div>
      </form>
    </Modal>
...
export function MergeDialog({
  ...
  onClose,
  commit,
}: Common & { data: InventoryData; item: Item }) {
  const [sourceId, setSourceId] = useState('');
  const source = data.items.find((item) => item.id === sourceId);
  const [localError, setLocalError] = useState<string | null>(null);
  const [dirty, setDirty] = useState(false);
  return (
    <Modal title="Merge a duplicate" onClose={onClose} busy={busy} dirty={dirty}>
      <p>
        Keep <strong>{item.name}</strong> and its unit, <strong>{item.unit}</strong>. The other item will be
        archived and its name kept as an alias.
      </p>
      <form
        onInput={() => setDirty(true)}
        onSubmit={async (event) => {
          ...
          try {
            ...
            setLocalError(error instanceof Error ? error.message : 'Review the merged values.');
          }
        }}
      >
        <Field label="Duplicate to combine">
          <select value={sourceId} onChange={(event) => setSourceId(event.currentTarget.value)} required>
            <option value="">Choose an item</option>
            {data.items
              .filter((other) => other.id !== item.id && !other.deleted_at)
              .sort((a, b) => a.name.localeCompare(b.name))
              .map((other) => (
                <option value={other.id}>
                  {other.name} · {number(other.quantity)} {other.unit}
                </option>
              ))}
          </select>
        </Field>
        {source && (
          <div class="comparison">
            <p>
              <strong>Other item:</strong> {number(source.quantity)} {source.unit},{' '}
        ...
        <Field
          label={`Actual combined quantity (${item.unit})`}
          hint="Count the shelf. The two records may describe the same stock."
        >
          <input
            name="quantity"
            type="number"
            ...
            step="any"
            required
            placeholder="Enter the total actually on hand"
          />
        </Field>
        <div class="form-grid">
        ...
        <label class="check-field">
          <input type="checkbox" name="never_prompt" defaultChecked={item.never_prompt} /> Never add
          automatically to shopping
        </label>
        <label class="check-field">
          <input type="checkbox" required /> I reviewed the units, quantity, and details for the combined
          item.
        </label>
        <ErrorMessage message={localError || error} />
        <div class="modal-actions">
          <button class="button primary" disabled={busy || !sourceId}>
            {busy ? 'Saving…' : 'Merge into ' + item.name}
          </button>
        </div>
      </form>
    </Modal>
