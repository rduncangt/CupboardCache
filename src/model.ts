...
const meta = {
  id: z.uuid(), created_at: timestamp, updated_at: timestamp, deleted_at: timestamp.nullable(),
};
...
const snapshotSchema = z.strictObject({ quantity, unit: z.string().min(1).max(40), package_size: packageSchema.nullable() });
...
const extraSchema = z.strictObject({ ...meta, text: z.string().trim().min(1).max(300), completed_at: timestamp.nullable() });
const settingsSchema = z.strictObject({ ...meta, history_retention_days: z.number().int().min(1).max(36500) });
const dataSchema = z.strictObject({
  ...meta,
  format: z.literal('cupboardcache'), schema_version: z.literal(1),
...
export function emptyInventory(now = new Date().toISOString()): InventoryData {
  return {
    ...metadata(now), format: 'cupboardcache', schema_version: 1, revision: 0,
    items: [], quantity_events: [], shopping_extras: [],
...
export function newItemInput(): ItemInput {
  return {
    name: '', description: null, aliases: [], category: null, location: 'Pantry',
    quantity: 1, unit: 'item', package_size: null, display_mode: 'numeric',
    expires_on: null, resupply_threshold: null, never_prompt: false,
  };
}

export function itemInput(item: Item): ItemInput {
  const { id: _id, created_at: _created, updated_at: _updated, deleted_at: _deleted,
    resupply_flag: _flag, last_checked_at: _checked, merged_into_id: _merged, ...input } = item;
  return input;
}
...
export function validateInventory(value: unknown): InventoryData {
  ...
  for (const record of [data, data.settings, ...data.items, ...data.quantity_events, ...data.shopping_extras]) {
    if (ids.has(record.id)) throw new Error('The backup contains duplicate record IDs.');
    ids.add(record.id);
    if (Date.parse(record.updated_at) < Date.parse(record.created_at)) throw new Error('A record was updated before it was created.');
  }
  if (data.deleted_at || data.settings.deleted_at) throw new Error('The inventory or its settings cannot be deleted.');
  const items = new Map(data.items.map(item => [item.id, item]));
  for (const item of data.items) {
    if (measures[item.unit] && item.package_size) throw new Error('An item counted in g, kg, ml, or l must not also redefine that unit with a package size.');
  ...
  for (const event of data.quantity_events) {
    ...
    if (event.reason !== 'initial' && !event.before) throw new Error('A quantity event is missing its previous amount.');
  }
  return data;
}
...
export function parseBackup(text: string): InventoryData {
  let value: unknown;
  try { value = JSON.parse(text); } catch { throw new Error('This file is not valid JSON. Choose a CupboardCache backup.'); }
...
export function matches(item: Item, search: string): boolean {
  ...
  return normalize(search).split(/\s+/).filter(Boolean).every(token => haystack.includes(token));
}
...
export function duplicates(items: Item[], name: string, excludeId?: string): Item[] {
  ...
  return items.filter(item => item.id !== excludeId && !item.merged_into_id && [item.name, ...item.aliases].some(alias => {
    const other = normalize(alias).split(' ').filter(Boolean);
    const overlap = tokens.filter(token => other.includes(token)).length;
    return normalize(alias) === normalize(name) || (tokens.length > 1 && overlap / Math.max(tokens.length, other.length) >= 0.75);
  })).slice(0, 5);
}

export function roundQuantity(value: number): number {
  if (!Number.isFinite(value) || value < 0 || value > 1_000_000_000) throw new Error('Quantity must be a nonnegative number no greater than 1 billion.');
  const rounded = Math.round((value + Number.EPSILON) * 1_000_000) / 1_000_000;
  if (value > 0 && rounded === 0) throw new Error('That amount is too small for this unit. Choose a smaller unit.');
  return rounded;
}

export const number = (value: number) => new Intl.NumberFormat(undefined, { maximumFractionDigits: 6 }).format(value);
export const snapshot = (item: Item) => ({ quantity: item.quantity, unit: item.unit, package_size: item.package_size ? { ...item.package_size } : null });
export const isLow = (item: Pick<Item, 'quantity' | 'resupply_threshold'>) => item.resupply_threshold !== null && item.quantity <= item.resupply_threshold;
...
export function assertUnitConversion(item: Item, factor: number, unit: string, pack: PackageSize | null): void {
  if (measures[unit] && pack) throw new Error('The new unit is already precise. Leave its package size blank.');
  const before = measures[item.unit] ?? (item.package_size ? { dimension: measures[item.package_size.unit].dimension, scale: item.package_size.amount * measures[item.package_size.unit].scale } : null);
  const after = measures[unit] ?? (pack ? { dimension: measures[pack.unit].dimension, scale: pack.amount * measures[pack.unit].scale } : null);
  if (before && after) {
    if (before.dimension !== after.dimension) throw new Error('Mass and volume cannot be converted into each other.');
    const expected = before.scale / after.scale;
    if (Math.abs(factor - expected) > Math.max(Math.abs(expected) * 0.000001, Number.EPSILON)) throw new Error(`The unit and package sizes require a conversion factor of ${number(expected)}.`);
  }
}
...
export function suggestedPackage(item: Item, factor: number, unit: string): PackageSize | null {
  ...
  if (measures[item.unit]) return { amount: measures[item.unit].scale / factor, unit: measures[item.unit].dimension === 'mass' ? 'g' : 'ml' };
  return null;
}

export function convertAmount(amount: number, fromUnit: string, item: Pick<Item, 'unit' | 'package_size'>): number {
  ...
  if (from && pack && from.dimension === measures[pack.unit].dimension) return roundQuantity(n * from.scale / (pack.amount * measures[pack.unit].scale));
...
export function entryUnits(item: Pick<Item, 'unit' | 'package_size'>): string[] {
  ...
  return [...new Set([item.unit, ...Object.keys(measures).filter(unit => measures[unit].dimension === measure?.dimension)])];
}

export function packageDisplay(item: Item): string | null {
  return item.package_size ? `${number(item.quantity * item.package_size.amount)} ${item.package_size.unit}` : null;
}

export function retentionCount(data: InventoryData, days: number, now = Date.now()): number {
  return data.quantity_events.filter(event => Date.parse(event.created_at) < now - days * 86_400_000).length;
}
