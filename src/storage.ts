...
const db = () =>
  openDB<Database>('cupboardcache', 1, {
    upgrade(database) {
...
const channel =
  typeof window !== 'undefined' && typeof BroadcastChannel !== 'undefined'
    ? new BroadcastChannel('cupboardcache-changes')
    : null;
...
export function execute(command: Command, expectedRevision: number): Promise<InventoryData> {
  return serial(() =>
    write(
      ...
      !['retention', 'prune'].includes(command.type),
    ),
  );
}
...
export function undoLast(expectedRevision: number): Promise<InventoryData> {
  return serial(() => {
    ...
    if (!previous || previous.revision !== expectedRevision)
      throw new Error('That action can no longer be undone.');
    return write(expectedRevision, (current) => undoCommand(current, previous.before), false);
  });
}

export async function restoreInventory(
  candidate: InventoryData,
  expectedRevision: number,
): Promise<InventoryData> {
  const checked = validateInventory(candidate);
  return serial(() =>
    write(expectedRevision, (current) => ({ ...checked, revision: current.revision + 1 }), false),
  );
}
...
export async function readStoredInventory(): Promise<unknown> {
  const database = await db();
  try {
    return await database.get('inventory', 'current');
  } finally {
    database.close();
  }
}
...
export function recoverInventory(candidate: InventoryData): Promise<InventoryData> {
  return serial(async () => {
    ...
    try {
      ...
      if (valid)
        throw new Error(
          'A readable inventory is now available. Close this dialog and refresh before restoring.',
        );
      const previousRevision =
        previous && Number.isSafeInteger(previous.revision) && previous.revision >= 0 ? previous.revision : 0;
      const restored = validateInventory({
        ...checked,
        revision: previousRevision < Number.MAX_SAFE_INTEGER ? previousRevision + 1 : 0,
      });
      ...
      try {
        ...
        /* Failed recovery leaves the existing data intact. */
      }
      await tx.done.catch(() => undefined);
      throw error;
    } finally {
...
export function storageError(error: unknown): string {
  if (error instanceof DOMException && error.name === 'QuotaExceededError')
    return 'This device is out of storage. Your last saved inventory is intact. Free some space and retry, or export a backup.';
