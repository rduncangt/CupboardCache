...
export function fixture(count = 8, eventCount = count): InventoryData {
  ...
  const names = ['Smoked paprika', 'Plain flour', 'Salted butter', 'Basmati rice', 'Chickpeas', 'Olive oil', 'Ground coffee', 'Frozen peas'];
  data.items = Array.from({ length: count }, (_, index) => ({
    ...newItemInput(), ...metadata(),
    name: count > 8 ? `Pantry item ${String(index).padStart(4, '0')}` : names[index],
    quantity: index % 3 === 0 ? 2.5 : index % 3 === 1 ? 0 : 2,
    unit: 'jar', category: index % 2 ? 'Everyday essentials' : 'Cooking',
    location: index === 2 ? 'Fridge' : index === 7 ? 'Freezer' : 'Pantry',
    display_mode: index % 3 === 0 ? 'qualitative' as const : 'numeric' as const,
    resupply_flag: index % 3 === 1, last_checked_at: null, merged_into_id: null,
  }));
  data.quantity_events = Array.from({ length: eventCount }, (_, index) => {
    const item = data.items[index % count];
    return { ...metadata(new Date(Date.now() - (index % 360) * 86_400_000).toISOString()), item_id: item.id,
      reason: 'recount' as const, before: snapshot(item), after: snapshot(item),
      related_item_id: null, undo_of_event_id: null, note: null };
  });
  return data;
}

export async function importFixture(page: Page, data: InventoryData) {
  await page.getByLabel('Choose backup file').setInputFiles({ name: 'test-inventory.json', mimeType: 'application/json', buffer: Buffer.from(JSON.stringify(data)) });
